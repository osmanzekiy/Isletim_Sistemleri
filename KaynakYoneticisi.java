    ...
    public synchronized boolean kaynaklariKontrolEt(Proses p) {
    	...
    	if(p.oncelik == 0 )
    	{
    		if((this.gercekZamanliBellek - p.bellek) < 0 || p.yaziciSayisi >0 ||
 	        ...
 	        {
 	        	 System.out.println("HATA!! Gerçek Zamanlı Proses Çok Fazla Kaynak Talep Ediyor - Proses Silindi: "+p.prosesSirasi);
 	        	 return false;
 	        }
 	        else
 	        	   return true;
    	...
    	else
    	{
    		if(p.yaziciSayisi > this.yaziciSayisi ||
 	               ...
 	               (this.kullaniciIsleriBellek - p.bellek) < 0)
 	        {
 	        	System.out.println("HATA!! Proses Çok Fazla Kaynak Talep Ediyor - Proses Silindi: "+ p.prosesSirasi);       	   
 	        	 return false;
 	        }
 	        else
